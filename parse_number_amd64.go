...
func parseNumber(buf []byte) (tag, val uint64) {
	pos := 0
	found := uint8(0)
	for i, v := range buf {
		t := isNumberRune[v]
	...
	if found&isFloatOnlyFlag == 0 && pos <= maxIntLen {
		...
		if err == nil {
			return uint64(TagInteger), uint64(i64)
		}
		if errors.Is(err, strconv.ErrRange) {
			flags |= uint64(FloatOverflowedInteger << JSONVALUEOFFSET)
		}

		if found&isMinusFlag == 0 {
			u64, err := strconv.ParseUint(unsafeBytesToString(buf[:pos]), 10, 64)
			if err == nil {
				return uint64(TagUint), u64
			}
			if errors.Is(err, strconv.ErrRange) {
				flags |= uint64(FloatOverflowedInteger << JSONVALUEOFFSET)
			}
	...
	if err == nil {
		return flags, math.Float64bits(f64)
	}
	return 0, 0
}
